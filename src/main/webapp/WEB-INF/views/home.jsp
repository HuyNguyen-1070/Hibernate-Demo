      ...
      <div class="bg-white p-3 rounded-4 shadow-sm">
        ...
        <div class="list-group list-group-flush">
          <c:forEach var="citem" items="${categories}">
            <a class="list-group-item ${citem.id.productCategoryId == activeCatId ? 'active' : ''}"
               href="<t:urlWithLang value='/home?cat=${citem.id.productCategoryId}'/>"> <div class="d-flex justify-content-between">
                <span>${citem.categoryName}</span> </div>
            </a>
          </c:forEach>
        </div>
      </div>
            ...
            <c:forEach var="p" items="${products}">
              <div class="col-md-6 col-xl-4">
                <div class="card h-100">
                  <div class="card-body">
                    <h6 class="card-title mb-1">${p.productName}</h6> <div class="muted small mb-2">#${p.product.productId}</div> <p class="muted" style="min-height:40px;"><c:out value="${p.productDescription}"/></p> <div class="d-flex justify-content-between">
                      <strong>₫${p.product.price}</strong> <span class="badge text-bg-light">⚖ ${p.product.weight} g</span> </div>
                  </div>
